...
select  T.pos, T.c_Tipo_GestioneProdotto, T.c_ramo_ministeriale,
		T.c_compagnia, T.n_polizza, T.n_posizione, T.c_prodotto, T.d_effetto,T.c_tipo_titolo,
...
if object_id('tempdb..#ESTR', 'U') is not null drop table #ESTR
select K.pos, K.nome_kpi, K.kpi, K.c_Tipo_GestioneProdotto, K.f_raggruppa_prodotto, K.c_tipo_liquidazione,C.Importolordo_titolo,C.Imposta_titolo,C.Importonetto_dettaglio,--K.tolleranza,
       c_garanzia                 = NULL,
	   ...
	   informazioni_aggiuntive    = cast(round(C.Importolordo_titolo,2) as money) ,
       oggetto_squadratura        = cast(round(C.Imposta_titolo,2) as money) + cast(round(C.Importonetto_dettaglio,2) as money)
into #ESTR
from #PE             K
...
update #ESTR set esito =  @ESITOOK,tipoesito = 'ESATTO' where informazioni_aggiuntive = oggetto_squadratura 
