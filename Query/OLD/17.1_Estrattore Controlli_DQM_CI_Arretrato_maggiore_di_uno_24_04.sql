...
join #STATO T on T.pos=D.pos and T.c_stato=S.c_stato
	...
	select K.pos, K.nome_kpi, K.kpi, K.c_Tipo_GestioneProdotto, K.f_raggruppa_prodotto, K.c_tipo_liquidazione,C.incassato,
		   ...
		   informazioni_aggiuntive    = C.d_effetto, --C.d_effetto_polizza,
	...
	/*
	select '#ESTR', * from #ESTR where esito = 'ko' and n_polizza in ( 		select n_polizza from Arretrato where c_esito <> 'A' )


	select '#ARR', * from #ARR  where n_polizza = 50011025869
	select 'Arretrato', c_tipo_titolo, * from Arretrato where n_polizza = 50011025869
